...
namespace Infoearth.Solution.MultiPointTile
{
    ...
    public class SqlHelper: IDbProvider
    {
        ...
        {
            return ConfigurationManager.ConnectionStrings["DbBase"].ConnectionString;
        }
        /// <summary>   
        /// 一个有效的数据库连接对象   
        /// </summary>   
        ...
        {
            return ExecuteNonQuery(GetConnSting(), CommandType.Text, commandText);
        }
        ...
        {
            object result = ExecuteScalar(GetConnSting(),commandText);
            if (result != null)
            {
        ...
        {
            return ExecuteDataset(GetConnSting(), CommandType.Text, $"select top {pageSize} * from {tableName} where unifiedcode not in (select top {pageSize * (pageIndex - 1)} id from {tableName})");
        }
